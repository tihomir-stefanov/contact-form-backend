<?php


namespace App\Tests\Functional\Controller;


use App\Tests\Integration\BaseFunctionalTest;
use Symfony\Component\HttpFoundation\Response;

/**
 * Class ContactFormControllerTest
 * @package App\Tests\Functional\Controller
 */
class ContactFormControllerTest extends BaseFunctionalTest
{
    /**
     *
     */
    public function testWithValidData()
    {
        $client = static::createClient();

        $client->request(
            'POST',
            '/contact-us',
            [],
            [],
            ['CONTENT_TYPE' => 'application/json'],
            '{"email":"valid@email.com", "message":"valid message"}'
        );

        /** @var Response $response */
        $response = $client->getResponse();
        $this->assertEquals(200, $response->getStatusCode());
        $this->assertTrue(
            $response->headers->contains(
                'Content-Type',
                'application/json'
            ),
            'the "Content-Type" header is "application/json"' // optional message shown on failure
        );
        $this->assertContains('"status":"success"', $response->getContent());
    }

    /**
     *
     */
    public function testWithMissingEmail()
    {
        $client = static::createClient();

        $client->request(
            'POST',
            '/contact-us',
            [],
            [],
            ['CONTENT_TYPE' => 'application/json'],
            '{"message":"valid message"}'
        );
        /** @var Response $response */
        $response = $client->getResponse();
        $this->assertEquals(200, $response->getStatusCode());
        $this->assertTrue(
            $response->headers->contains(
                'Content-Type',
                'application/json'
            ),
            'the "Content-Type" header is "application/json"' // optional message shown on failure
        );
        $this->assertContains('"status":"error"', $response->getContent());
    }

    /**
     *
     */
    public function testWithMissingMessage()
    {
        $client = static::createClient();

        $client->request(
            'POST',
            '/contact-us',
            [],
            [],
            ['CONTENT_TYPE' => 'application/json'],
            '{"email":"valid@email.com"}'
        );
        /** @var Response $response */
        $response = $client->getResponse();
        $this->assertEquals(200, $response->getStatusCode());
        $this->assertTrue(
            $response->headers->contains(
                'Content-Type',
                'application/json'
            )
        );
        $this->assertContains('"status":"error"', $response->getContent());
    }


    /**
     * @dataProvider getInvalidData
     */
    public function testWithInvalidData(string $email, string $message)
    {
        $client = static::createClient();

        $client->request(
            'POST',
            '/contact-us',
            [],
            [],
            ['CONTENT_TYPE' => 'application/json'],
            '{"email":"' . $email . '","message":"' . $message . '"}'
        );
        /** @var Response $response */
        $response = $client->getResponse();
        $this->assertEquals(200, $response->getStatusCode());
        $this->assertTrue(
            $response->headers->contains(
                'Content-Type',
                'application/json'
            )
        );
        $this->assertContains('"status":"error"', $response->getContent());
    }

    /**
     * @return array
     */
    public function getInvalidData()
    {
        return [
            ['not-actual-email', 'Valid message'],
            ['', 'Valid message'],
            ['long_email1NL2q52MnF8MNvC6rnPJlHw5tHiF71qaE6xmRCuwT1p7TgiZNmxlYpwsl9nhsFHoInfiV0JwuSIqld36lKp3uD69FHbu3NyPzwamb3YZf34FwnGzoexk5R9UsjAKJa51D63IM8xfxhstH9E8uwR8YPP7aPrSlyx4ZD4yf5vAoI7E4T1hRYdEyf6Vj3dF8V5a5uPiynEC7HyBJI7gMbvPTfIlEMA0MfF8cO47QvvO2c1QsCuIxQRI@long.com', 'Valid message'],
            ['valid@email.com', ''],
            ['valid@email.com', '1001_'],
        ];
    }
}
